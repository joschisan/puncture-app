...
use puncture_api_core::AppEvent;
...
impl PunctureClientWrapper {
    ...
    #[flutter_rust_bridge::frb]
    pub async fn add_daemon(&self, invite: &str) -> Result<PunctureConnectionWrapper, String> {
        Ok(PunctureConnectionWrapper(
            self.0.add_daemon(invite.to_string()).await?,
        ))
    }

    /// Get list of daemons
    #[flutter_rust_bridge::frb(sync)]
    pub fn get_daemons(&self) -> Vec<DaemonWrapper> {
        self.0
            .get_daemons()
            .into_iter()
            .map(DaemonWrapper)
...
impl PaymentRequestWithAmountWrapper {
    ...
    pub fn display(&self) -> String {
        match &self.0 {
            PaymentRequestWithAmount::Bolt11(..) => "Bolt11 Invoice".to_string(),
            PaymentRequestWithAmount::Bolt12(..) => "Bolt12 Offer".to_string(),
        }
    }
